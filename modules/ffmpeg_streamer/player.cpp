...
void *_run_decoder(void *arg) {
  MediaPlayerContext *vpc = (MediaPlayerContext*)arg;
...
MediaPlayerContext *player_create(const char *url, uint8_t convert_to_rgb) {
  ...
  vpc->url = (char*)malloc(strlen(url) + 1);
  strcpy(vpc->url, url);
...
double player_grab_audio_frame(MediaPlayerContext *vpc, void **release_ptr, uint8_t **data, int *frame_size, int *format) {
  if (vpc->dectx == NULL) return 0.0;
...
void player_release_frame(MediaPlayerContext *vpc, void *release_ptr) {
  AVFrame *frame = (AVFrame*)release_ptr;
  av_frame_free(&frame);
}
