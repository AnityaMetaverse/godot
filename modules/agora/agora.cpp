...
// void AgoraClient::register_account() {
// 	rtc_engine->registerLocalUserAccount(app_id.ascii().get_data(), username.ascii().get_data());
// }
...
{
	username_id.insert(username, user_id);
}

bool AgoraClient::is_muted(const String& username) const
{
	// rtc_engine->get
}
...
void RTCEventHandler::onUserInfoUpdated(uid_t uid, const UserInfo& info)
{
	client->add_user(String(info.userAccount), user_id);
}
